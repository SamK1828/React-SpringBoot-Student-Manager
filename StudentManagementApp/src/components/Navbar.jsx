...
const Navbar = () => {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        if (searchQuery.trim()) {
            // Navigate to Students page with query param
            navigate(`/students?name=${encodeURIComponent(searchQuery)}`);
            setSearchQuery("");
        }
    };
    ...
    return (
        <nav className="sticky top-0 z-50 bg-gradient-to-r from-blue-800 via-indigo-700 to-purple-700 text-white shadow-lg">
            <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-white/80">
                            Manage students, courses & progress
                        </div>
                    </div>
                </div>
                ...
                {/* Desktop navigation */}
                <div className="hidden md:flex items-center space-x-6">
                    <Link to="/" className="px-3 py-2 rounded-md hover:bg-white/10">
                        Home
                    </Link>
                    <Link to="/students" className="px-3 py-2 rounded-md hover:bg-white/10">
                        Students
                    </Link>
                    <Link to="/about" className="px-3 py-2 rounded-md hover:bg-white/10">
                        About
                    </Link>
                </div>
                ...
                <div className="hidden md:flex items-center space-x-4">
                    ...
                    <form onSubmit={handleSearch} className="relative">
                        ...
                        />
                        <svg
                            className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-white/80"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                        >
                ...
                <div className="md:hidden">
                    <details className="relative">
                        <summary className="p-2 rounded-md hover:bg-white/10 cursor-pointer">
                            <svg
                                className="w-6 h-6"
                                fill="none"
                                stroke="currentColor"
                        ...
                        <div className="mt-2 bg-white/5 rounded-md p-3 space-y-2">
                            <Link to="/" className="block px-3 py-2 rounded-md hover:bg-white/10">
                                Home
                            </Link>
                            <Link
                                to="/students"
                            ...
                            <Link
                                to="/add-student"
                                className="block mt-1 bg-white text-indigo-700 px-3 py-2 rounded-md text-center"
                            >
                                + Add Student
                            </Link>
                        </div>
